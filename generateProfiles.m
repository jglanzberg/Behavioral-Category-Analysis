function [profile,raw_distances] = generateProfiles(rois,tracking_data)
...
%% Assign which version of the tracking data will be used here

% head_pos = tracking_data.trk_filt;
...
for x = 1:numel(head_pos)
    if ~isempty(head_pos{x})
        ...
        head_pos{x} = [ head_pos{x};nan(1100 - size( head_pos{x},1),15,3)];
        head_pos{x} = head_pos{x}(:,head_pieces,1:2);

        
        
        body_pos{x} = cat(3,body_pos{x}.x,body_pos{x}.y,body_pos{x}.p);
        body_pos{x} = [ body_pos{x};nan(1100 - size( body_pos{x},1),15,3)];
        ...
        velocity_b{x} = [nan(1,1);vecnorm(median(movmean(diff(body_pos{x},1,1),11,1,'omitnan'),2,'omitnan'),2,3)];
         ...
         % active{x} = [nan(1,1);vecnorm(median(movmax(diff(head_pos{x},1,1),21,1,'omitnan'),2,'omitnan'),2,3)]>5;
       
         % seems like 15 is a better ths
        active{x} = [nan(1,1);median(movmax(vecnorm(diff(head_pos{x},1,1),2,3),21,1,'omitnan'),2,'omitnan')];


        % slowing{x} = [nan(1,1);min(vecnorm(movmin(diff(body_pos{x},1,1),5,1,'omitnan'),2,3),[],2,"omitmissing")];
...
% ths_slowing = cellfun(@(X) X <= 3,slowing);
% ths_active = cellfun(@(X) X >= 10,active);
...
locomotion = cellfun(@(X) X > 0.7,velocity_b,'UniformOutput',false);
resting = cellfun(@(X) [zeros(98,1);utils.keepConsecutiveTrue(~X(99:1051),min_resting_len);zeros(49,1)],locomotion,'UniformOutput',false);
...
s2lp = registration_code.eventVector(tracking_data.lvr_food,tracking_data.timestamps,-20,1100,0);
...
retr_1 = registration_code.eventVector(tracking_data.retrieval,tracking_data.timestamps,10,1100,0);
%retrieval



% Consumption phase
consump = registration_code.eventVector(tracking_data.retrieval,tracking_data.timestamps,90,1100,10);
...
%Hl off
HL_on = profiles.twoPointVector(tracking_data.HL_on,tracking_data.HL_off,1100);
...
%Other
profile(13,:) = cellfun(@(X) ones(1100,1),head_pos(~cellfun(@isempty,head_pos)),'UniformOutput',false);
